...
class PreTrainedTokenizerBase(PushToHubMixin):
    ...
    def parse_response(
        ...
        *,
        ...
        Currently accepts a single sequence only; for batched inputs, call `parse_response` per item.

        Args:
            response (`str`, token ids or 1D tensor):
                The output generated by the model. Either decoded text or a single sequence of token ids
                (list / 1D array / 1D tensor). Note that this should contain only model output, not any preceding
            ...
            prefix (`str`, token ids or 1D tensor, *optional*):
                The prompt that came before generation. This is necessary because many chat templates
                 pre-write part of the message, so we need to see the prompt to parse correctly.
        """
        ...
        if use_new_template:
            return _template_parse_response(response, schema, prefix=prefix)
